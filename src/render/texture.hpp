...
public:
    ...
    // the format param is straight up not used anymore but i lowk cant be bothered to remove it everywhere so i'm just not gonna (its safe to have this as NULL or 0.)
    ...
    static Texture* createNewTextureFromPath(std::string  path, unsigned int filterMode = DEFAULT_FILTERING, unsigned int format = 0, unsigned int textureType = GL_TEXTURE_2D, bool serialize = true);
    static Texture* createNewTextureWithData(unsigned char* data, unsigned int filterMode = DEFAULT_FILTERING, unsigned int format = 0, unsigned int textureType = GL_TEXTURE_2D, bool serialize = true);
    static Texture* getWithID(unsigned int id);

    glm::vec2 dimensions() { return m_data.m_size; };
